...
$$
\begin{aligned}
&A_{z} := \oplus_{i\geq 0} A_iz^{i}.
\end{aligned}
$$
 The $k[z]$-algebra $A_{z}$ is finitely generated over $k$ by  elements
 corresponding to the minimal generators of $S$, and flat over $k[z]$ because it is torsion free. 
 The formulas 
 $$
 k[z,z^{-1}]\otimes_{k[z]} A_{z} = k[z,z^{-1}]\otimes_{k}  A_{z} 
 $$
 and 
 $$ 
 k[z]/(z)\otimes_{k[z]} \tilde A  = \oplus A_{i}/A_{i+1} = k[S]
 $$ 
