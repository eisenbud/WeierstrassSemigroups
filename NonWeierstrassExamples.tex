...
The main contribution of this paper is a condition  on the free resolution of a semigroup ideal that forces every positively graded deformation to have a certain multi-valued
...
{\bf NumericalSemigroups.m2} \cite{NumericalSemigroupsSource} and {\bf WeierstrassSemigroups.m2} \cite{WeierstrassSemigroupsSource}
...
\begin{proof}
Suppose $S$ is the Weierstrass semigroup of a point $p$ on smooth projective curve $C$.
Consider the line bundle $\sO_{C}(p)$ and the graded ring
$$
A= \bigoplus_{i\ge 0} H^0(C,\sO_{C}(ip)).
$$
As an $k$-algebra $A$ is generated by elements of degree $s_{i}$ corresponding to generators of the semigroup $S$ and the element 
...
shows that $S$ is not Weierstrass. Conversely if there are singular points in every fiber then they define a multi-section. Thus in proving that a semigroup is not Weierstrass, it is natural to look for a multisection. However there are also families with section where the section actually follows
...
\begin{example}\label{smooth section}
 As explained in the introduction, we could search for semigroups that are not Weierstrass by looking for those moving in families with a multi-section. However, there are positively graded deformations of semigroup rings that admit a section, but where the image of the section
...
Pinkham's Theorem  \cite{MR376672} and 
 computed explicit 1-parameter positively graded smoothing families  over $\QQ$ directions for each of the 632 examples that was not already known. This shows that they are Weierstrass semigroups over $\QQ$, and thus over $\CC$. 

We exhibit these families
In the auxiliary file to the arXiv preprint of this paper  
...
Restricting to deformation parameters to a certain range of degrees, for example degrees $>b$, can make the computation
