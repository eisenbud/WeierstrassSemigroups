...
Weierstrass semigroups. In an auxiliary file we give explicit positively graded smoothing families
...
be Weierstrass, both using Buchweitz's method and otherwise; see for example 
 ...
 \cite{MR3010539}. Examples by Komeda \cite{MR3010539} had, until now, the
...
\frank {alternative statement and proof}

\begin{proposition}[Pinkham] \label{full Pinkham}
Let $S$ be a numerical semigroup of genus $g$. $S$ is a Weierstrass semigroup if and only if there exists a one-parameter positively graded homogeneous smoothing of the semigroup  ring $k[S]$ of $S$.
\end{proposition}
...
Adjusting the generators we see that $A/z \cong k[S]$. Since $C = \Proj A$ is smooth the open subset, where $z\not=0$ is smooth, and since
...
\xymatrix{ \Spec k[S] \ar[r] \ar[d] & \Spec A \ar[d]\cr
 ...
 Then $C=\Proj A$ is a smooth projective curve in a weighted projective space with a unique point $p$ at infinity, which has $S$ as Weierstrass semigroup. Note that $C$ is smooth because the singular point of the special fiber of the affine curve $\Spec k[S]$  corresponds to the homogeneous maximal ideal of $A$.
 \end{proof}
...
\frank{ add a Pinkham's Theoreme13.5 perhaps in the formulation of Jan Stevens?
Are their reducible $M_{g,1}^{S}$ known?}
